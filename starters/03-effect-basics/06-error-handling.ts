...
section(
	'Four ways to deal with the E channel: recover, fall back, match both sides, or capture as a value.'
)
...
// ---- checks (don't edit) ----
check(
	'catch/orElse recovers a failure to the default',
	Effect.runSync(safe(-5)),
	0
)
check(
	'catch/orElse leaves a success untouched',
	Effect.runSync(safe(7)),
	7
)
check(
	'match collapses the failure side',
	Effect.runSync(described(-1)),
	'error: negative'
)
check(
	'match collapses the success side',
	Effect.runSync(described(3)),
	'ok: 3'
)
const r = Effect.runSync(captured(-2))
check(
	'result captures failure as a value (no throw)',
	Result.isFailure(r),
	true
)
check(
	'the captured Result holds the error',
	Result.isFailure(r) ? r.failure : null,
	'negative'
)
