...
import { type CreateTodo, type Todo, TodoNotFound, type UpdateTodo } from "./domain.ts";
...
export class TodoRepo extends Context.Service<
  TodoRepo,
  {
    ...
    readonly update: (id: number, input: UpdateTodo) => Effect.Effect<Todo, TodoNotFound>;
    readonly remove: (id: number) => Effect.Effect<void, TodoNotFound>;
  }
>()("TodoRepo") {}

type State = { readonly nextId: number; readonly todos: ReadonlyArray<Todo> };
...
export const TodoRepoMemory = Layer.effect(TodoRepo)(
  Effect.gen(function* () {
    ...
    return TodoRepo.of({
      ...
      create: (input) =>
        Ref.modify(ref, (s) => {
          const todo: Todo = { id: s.nextId, title: input.title, done: false };
          return [todo, { nextId: s.nextId + 1, todos: [...s.todos, todo] }];
        }),
