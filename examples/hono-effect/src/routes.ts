...
const decode = <A>(
  ...
  Schema.decodeUnknownEffect(schema)(input).pipe(
    Effect.mapError(
      (issue) =>
        new InvalidInput({ message: String(issue.message).split("\n")[0]! }),
    ),
  );
...
const handleErrors = <R>(
  ...
  self.pipe(
    Effect.catchTags({
      TodoNotFound: (e) =>
        Effect.succeed(
          Response.json({ error: "TodoNotFound", id: e.id }, { status: 404 }),
        ),
      InvalidInput: (e) =>
        Effect.succeed(
          Response.json(
...
const parseId = (raw: string) =>
  /^\d+$/.test(raw)
    ...
    : Effect.fail(new InvalidInput({ message: `"${raw}" is not a valid id` }));
