...
export const CreateTodo = Schema.Struct({ title: Schema.NonEmptyString });
  ...
  Effect.gen(function* () {
    const ref = yield* Ref.make({
      ...
      todos: [
        ...
        { id: 2, title: "Use Effect in a SvelteKit load", done: false },
...
export const decode = <A>(
  ...
  Schema.decodeUnknownEffect(schema)(input).pipe(
    Effect.mapError(
      (issue) => new InvalidInput({ message: String(issue.message).split("\n")[0]! }),
    ),
  );
