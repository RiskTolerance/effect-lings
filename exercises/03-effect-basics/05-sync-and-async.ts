...
const parse = (s: string): Effect.Effect<unknown, string> => Effect.succeed(null); // fix me

// 📝 TODO: wrap a promise with Effect.tryPromise, mapping a rejection to "bad".
const fromPromise = (p: Promise<number>): Effect.Effect<number, string> => Effect.succeed(0); // fix me

// ---- checks (don't edit) ----
check("sync is lazy: defining tick did not run it", calls, 0);
check("running tick twice bumps the counter", [Effect.runSync(tick), Effect.runSync(tick)], [1, 2]);
check("try maps a parse success", Effect.runSync(parse("[1,2,3]").pipe(Effect.map((v) => (v as number[]).length))), 3);
check("try maps a throw to a typed error", Effect.runSync(parse("nope").pipe(Effect.catch((e) => Effect.succeed(e)))), "bad json");
check("tryPromise resolves to the value", await Effect.runPromise(fromPromise(Promise.resolve(9))), 9);
check("tryPromise maps a rejection", await Effect.runPromise(fromPromise(Promise.reject("x")).pipe(Effect.catch((e) => Effect.succeed(e)))), "bad");
