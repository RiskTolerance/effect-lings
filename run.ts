...
type Progress = { completed: string[]; streak: number; lastCompletedDate: string };

function loadProgress(): Progress {
  if (!existsSync(progressFile)) return { completed: [], streak: 0, lastCompletedDate: "" };
  try {
...
function allExercises(): string[] {
  return readdirSync(exercisesDir, { recursive: true, withFileTypes: true })
    .filter((d) => d.isFile() && d.name.endsWith(".ts"))
    .map((d) => relative(exercisesDir, join((d as any).parentPath ?? (d as any).path, d.name)))
    .sort();
}
...
function runFile(rel: string): number {
  const res = spawnSync(bunBin, [join(exercisesDir, rel)], { stdio: "inherit" });
  return res.status ?? 1;
}
...
function cmdToday(): void {
  ...
  if (!next) {
    ...
    console.log(`Add another file under exercises/ and keep the streak alive (${p.streak} days).\n`);
    return;
  }
  ...
  if (status === 0) {
    ...
    if (wasNew) {
      ...
      console.log(`🔥  ${p.streak}-day streak  ·  ${p.completed.length}/${all.length}  ${bar(p.completed.length, all.length)}`);
      console.log(upcoming ? `Next: ${pretty(upcoming)} — run \`bun start\` again when you're ready.\n` : `That was the last one. 🎉\n`);
    }
  } else {
    console.log(`\n— not passing yet. Edit exercises/${next}, then run \`bun start\` again.`);
    console.log(`   (Or \`bun run watch\` to re-check automatically on every save.)\n`);
    process.exitCode = 1;
  }
}
...
function cmdWatch(): void {
  ...
  console.log(`   When it goes green, stop and run \`bun start\` to record it.\n`);
  spawnSync(bunBin, ["--watch", join(exercisesDir, next)], { stdio: "inherit" });
}
...
function cmdList(): void {
  ...
  for (const e of all) {
    ...
    console.log(`    ${done.has(e) ? "✓" : "·"} ${e.split(sep).slice(1).join(sep).replace(/\.ts$/, "")}`);
  }
  console.log("");
}
...
({ today: cmdToday, watch: cmdWatch, list: cmdList, reset: cmdReset }[cmd] ?? cmdToday)();
